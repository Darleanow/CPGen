 ...
 * @brief Manages persistent user preferences loaded from a JSON configuration file.
 * @author Darleanow
 */
 ...
 */
struct PreferencesConfig {
  bool support_modern_font = false; ///< Whether the terminal font supports Nerd Font / Unicode glyphs.
};
...
public:
  /**
   * @brief Construct a @c PreferencesManager, optionally overriding the config path.
   * @param config_path Custom path to the JSON preferences file.  When empty
   *        (@c std::nullopt) the default path @c ~/.config/CPGen/config.json is used.
   */
   ...
   *         Returns default-initialised values if the config has not been read yet.
   */
  [[nodiscard]] const PreferencesConfig &getConfig() const;

private:
  std::filesystem::path m_config_path{"~/.config/CPGen/config.json"}; ///< Resolved path to the JSON config file.
  PreferencesConfig m_preferences; ///< In-memory representation of the loaded preferences.

  /**
   * @brief Read and deserialise the JSON configuration file into @ref m_preferences.
